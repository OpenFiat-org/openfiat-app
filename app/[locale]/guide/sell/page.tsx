...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "guide.sell" });
  return {
...
export default function SellGuidePage() {
  const t = useTranslations("guide.sell");
  ...
  return (
    <section>
      ...
      <div className="mt-10 max-w-3xl">
        ...
        <h2 className="mt-10 text-sm font-semibold uppercase tracking-wider text-gray-400">{t("staySafeHeading")}</h2>
        ...
        <div className="mt-10 flex flex-wrap gap-6 border-t border-white/10 pt-6 text-sm">
          <Link href="/" className="text-brand hover:text-brand-hover">{t("startSelling")}</Link>
          <Link href="/guide/buy" className="text-brand hover:text-brand-hover">{t("buyingInstead")}</Link>
        </div>
      </div>
    </section>
  );
